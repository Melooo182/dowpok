----------------------------------------
-- File: 'weapon\space_marine_hurricane_bolter_right.lua'
-- Created by: AttributeEditor v2.0
-- Note: Do NOT edit by hand!
-- (c) 2001 Relic Entertainment Inc.

GameData = Inherit([[weapon\space_marine_hurricane_bolter.nil]])
MetaData = InheritMeta([[weapon\space_marine_hurricane_bolter.nil]])

GameData["accuracy_reduction_when_moving"] = 0.00000
GameData["cost"]["cost"]["requisition"] = 50.00000
GameData["cost"]["time_seconds"] = 10.00000
GameData["horizontal_traverse_speed"] = 90.00000
GameData["max_traverse_up"] = 60.00000


MetaData["$METADATATAG"] = {desc = [[]], type = 7, category = [[]], dispval = [[]], isLocked = false, }
MetaData["$METACOLOURTAG"] = 
{

}
