----------------------------------------
-- File: 'ebps\races\dark_eldar\troops\dark_eldar_support_slave_murdered.lua'
-- Created by: AttributeEditor v2.0
-- Note: Do NOT edit by hand!
-- (c) 2001 Relic Entertainment Inc.

GameData = Inherit([[ebps\races\dark_eldar\troops\dark_eldar_support_slave.lua]])
MetaData = InheritMeta([[ebps\races\dark_eldar\troops\dark_eldar_support_slave.lua]])

GameData["cost_ext"]["time_cost"]["cost"]["requisition"] = 0.00000
GameData["cost_ext"]["time_cost"]["time_seconds"] = 21.00000
GameData["entity_blueprint_ext"]["animator"] = "Environment/Gameplay/Cannibalism_Parts"
GameData["health_ext"]["hitpoints"] = 0.00000
GameData["health_ext"]["regeneration_rate"] = -5.00000
GameData["ui_ext"]["speech_directory"] = ""
GameData["ui_ext"]["ui_hotkey_name"] = "dark_eldar_murdered_slave"
GameData["ui_ext"]["ui_index_hint"] = 9.00000
GameData["ui_ext"]["ui_info"]["help_text_list"]["text_01"] = "$4100095"
GameData["ui_ext"]["ui_info"]["help_text_list"]["text_02"] = "$0"
GameData["ui_ext"]["ui_info"]["help_text_list"]["text_03"] = "$0"
GameData["ui_ext"]["ui_info"]["screen_name_id"] = "$4100094"


MetaData["$METADATATAG"] = {desc = [[]], type = 7, category = [[]], dispval = [[]], isLocked = false, }
MetaData["$METACOLOURTAG"] = 
{

}
